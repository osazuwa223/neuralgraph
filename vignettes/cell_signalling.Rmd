...
The ultimate goal of network inference is to predict novel edges.  Suppose a novel edge A -> B exists.  The causal network learning algorithm, as described above, ranks that edge with model averaging; it assigns a weight based on how frequently it appears across as set of individual learned networks.   The weight of the edge depends on what proteins are measured.  Firstly, A and B must both be measured in the input data or the edge obviously can't be detected and is effectively assigned a weight of 0.  Secondly, the measurement (or lack thereof) of other proteins affect the likelihood calculation of an edge, and thus its presence in learning results.   

Therefore, the method of selecting proteins for measurement determines performance in detecting a novel edge with causal network learning. 
...
```{r}
library(ggplot2)
data_sparcity <- c("15% null edges", "30% null edges", "75% null edges")
data_noise <- c("low noise", "medium noise", "high noise")
data_coverage <- c("data on 10% of nodes", "50% of nodes", "80% of nodes")
network_density <- c("5% net density", "15% net density", "50% net density")
network_size <- c("10 node net", "25 node net", "55 node net")
sim_results <- tcells$sim_results 
my_data <- sim_results %>%
  mutate(data_sparcity = factor(sample(data_sparcity, nrow(.), replace = TRUE), levels = data_sparcity),
         data_noise = factor(sample(data_noise, nrow(.), replace = TRUE), levels = data_noise),
         data_coverage = factor(sample(data_coverage, nrow(.), replace = TRUE), levels = data_coverage),
         network_density = factor(sample(network_density, nrow(.), replace = TRUE), levels = network_density),
         network_size = factor(sample(network_size, nrow(.), replace = TRUE), levels = network_size))
g_plot <- ggplot(my_data, aes(x = score)) +
  ...
  facet_grid(network_density ~ data_noise)
plot(g_plot)
g_plot <- ggplot(my_data, aes(x = score)) +
  geom_density(aes(fill=approach ), alpha = 0.5) + 
  labs(title = "Distributions of Novel Edge Scores") + 
  scale_x_continuous(breaks = c(0, .25, .5, .75)) +
  facet_grid(network_size ~ data_coverage)
plot(g_plot)
```
